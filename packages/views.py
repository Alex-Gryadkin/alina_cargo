    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if UserPackages.objects.filter(user_id=request.user).filter(package_id=thispack).count()==0:
                ...
                return JsonResponse({'errorMessage': 0, 'packageid': package.package_id.id, 'desc': package.desc, 'status': package.package_id.get_status_display(), 'changedate': package.package_id.status_change_date.strftime("%d.%m.%Y %H:%M")}, status=200)
            else:
